...
function _simg($filename, $dim = '640x480', $cut_enabled=false)
{
    ...
    {
        $filename_encode = basename('adminudora-assets/img/no_image.jpg');
    }
...
function btn_edit_udora($uri)
{
	return anchor($uri, '<i class="fa fa-edit"></i> '.lang('edit'), array('class'=>''));
}
...
function btn_delete_udora($uri)
{
	return anchor($uri, '<i class="fa fa-close"></i> '.lang('delete'), array('onclick' => 'return confirm(\''.lang('Are you sure?').'\')', 'class'=>''));
}
...
function generate_treefields_list($treefield_id = 64, $lang_id= NULL) {
    static $_gen_treefields;
    
    if(!empty($_gen_treefields[$treefield_id])) {
        return $_gen_treefields[$treefield_id];
    }
    ...
    if($CI->session->userdata('type')!='ADMIN'){
        $cacher_tree = $CI->cacher_m->load('treefield_'.$treefield_id);
        if($cacher_tree) {
            $_gen_treefields[$treefield_id] = unserialize($cacher_tree);
            return $_gen_treefields[$treefield_id];
        }
    }
    ...
    $CI->db->query('ALTER TABLE `treefield` ADD `font_icon_code` VARCHAR(64) NOT NULL AFTER `notifications_sent`;
...
');
    ...
    foreach ($_treefields as $val) {
        ...
        if (!empty($options->{'body'})) {
            ...
            $href = site_url($lang_code . '/' . get_results_page_id() . '/?search={"v_search_option_' . $treefield_id . '":"' . rawurlencode($treefield['title'] . ' - ') . '"}');
            $treefield['url'] = $href;
        }
        /* end if have body */
        ...
        if (isset($tree_listings[$val->id]) && sw_count($tree_listings[$val->id]) > 0)
            foreach ($tree_listings[$val->id] as $key => $_child) {
                ...
                if (!empty($options->{'body'})) {
                    ...
                    $href = site_url($lang_code . '/' . get_results_page_id() . '/?search={"v_search_option_' . $treefield_id . '":"' . rawurlencode($treefield['title'] . ' - ' . $child['title'] . ' - ') . '"}');
                    $child['url'] = $href;
                }
                /* end if have body */
    ...
    $_gen_treefields[$treefield_id] = $treefields;
    // cache results 
    $CI->cacher_m->cache_update('treefield_'.$treefield_id, serialize($treefields), 7);
