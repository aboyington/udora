...
class Frontend extends Frontend_Controller
{
    ...
    public function myattended()
    {
        ...
        // Main page data
        $this->data['page_navigation_title'] = lang_check('My attended');
        $this->data['page_title'] = lang_check('My attended');
        $this->data['page_body']  = '';
        $this->data['page_description']  = '';
        $this->data['page_keywords']  = '';
        ...
        $output = $this->parser->parse($this->data['settings_template'].'/myattended.php', $this->data, TRUE);
        echo str_replace('assets/', base_url('templates/'.$this->data['settings_template']).'/assets/', $output);
    }
    ...
    public function myevents()
    {
        ...
        // Main page data
        $this->data['page_navigation_title'] = lang_check('My events');
        $this->data['page_title'] = lang_check('My events');
        $this->data['page_body']  = '';
        $this->data['page_description']  = '';
        $this->data['page_keywords']  = '';
        ...
        $output = $this->parser->parse($this->data['settings_template'].'/myevents.php', $this->data, TRUE);
        echo str_replace('assets/', base_url('templates/'.$this->data['settings_template']).'/assets/', $output);
    }
        ...
        if($this->form_validation->run() == TRUE)
        {
            ...
            if($this->data['user_data']['mail'] != $data['mail'] && config_db_item('email_activation_enabled') === TRUE)
            {
                ...
                if(!empty($data['mail']))
                {
                    ...
                    $config_mail['mailtype'] = 'html';
                    $this->email->initialize($config_mail);
                    $this->email->from($this->data['settings_noreply'], lang_check('Your friends at Udora'));
    ...
    public function notificationsettings()
    {
        ...
        // Main page data
        $this->data['page_navigation_title'] = lang_check('Notification Settings');
        $this->data['page_title'] = lang_check('Notification Settings');
        $this->data['page_body']  = '';
        $this->data['page_description']  = '';
        $this->data['page_keywords']  = '';
        ...
        $rules = $this->user_m->rules_admin;
        unset($rules['type'], $rules['language']);
        unset($rules['username'], $rules['mail'], $rules['name_surname']);
        //$rules = array();
        $this->form_validation->set_rules($rules);
        ...
        if($this->form_validation->run() == TRUE)
        {
            if($this->config->item('app_type') == 'demo')
            {
                $this->session->set_flashdata('error', 
                        lang('Data editing disabled in demo'));
                redirect('frontend/notificationsettings/'.$this->data['lang_code'].'/');
                exit();
            }
            
            $data = $this->user_m->array_from_post(array('favorites_notifications', 'reviews_notifications', 'promotional_emails', 'information_disclosed'));
            
            
            $message_mail = '';
            if($this->data['user_data']['mail'] && config_db_item('email_activation_enabled') === TRUE)
            {
                $data['mail_verified'] = 0;
                // [START] Activation email
                
                if(ENVIRONMENT != 'development')
                if(!empty($this->data['user_data']['mail']))
                {
                    $this->load->library('email');
                    $config_mail['mailtype'] = 'html';
                    $this->email->initialize($config_mail);
                    $this->email->from($this->data['settings_noreply'], lang_check('Your friends at Udora'));
                    $this->email->to($this->data['user_data']['mail']);
                    
                    $this->email->subject(lang_check('Please confirm your email address'));
                    
                    $new_hash = substr($this->user_m->hash($this->data['user_data']['mail'].$this->data['user_data']['id']), 0, 5);
            ...
            if($this->data['user_data']['phone'] && !empty($this->data['user_data']['phone']) &&
               config_db_item('clickatell_api_id') != FALSE && config_db_item('phone_verification_enabled') === TRUE &&
               file_exists(APPPATH.'libraries/Clickatellapi.php'))
            {
                $data['phone_verified'] = 0;
                ...
                //Send SMS for phone verification
                $new_hash = substr($this->user_m->hash($this->data['user_data']['phone'].$this->data['user_data']['id']), 0, 5);
                ...
                $this->load->library('clickatellapi');
                $return_sms = $this->clickatellapi->send_sms($message, $this->data['user_data']['phone']);
        ...
        $output = $this->parser->parse($this->data['settings_template'].'/notification_settings.php', $this->data, TRUE);
        echo str_replace('assets/', base_url('templates/'.$this->data['settings_template']).'/assets/', $output);
    }
    ...
    public function editproperty()
    {
        ...
        if($user->package_id > 0)
        {
            ...
            if(isset($listing_num[$user->id]))
            {
                ...
                else if($package->package_days > 0 && strtotime($user->package_last_payment)<=time())
                {
                    ...
                    redirect('frontend/myevents/'.$this->data['lang_code'].'#content');
                    exit();
                }
            }
        }
	    ...
	    if($id)
        {
            ...
            if(count($this->data['estate']) == 0)
            {
                ...
                redirect('frontend/myevents/'.$this->data['lang_code'], 'refresh');
            }
...
//                                                                ), true, 1, 'package_last_payment DESC');
            ...
            if(isset($repository_id) && isset($this->data['files'][$repository_id]) && count($this->data['files'][$repository_id]) > 0)
            {
                ...
                redirect('frontend/myevents/'.$this->data['lang_code']);
            }
            else
            {
            $this->session->set_flashdata('message', 
                ...
                try {
                    ...
                    if(count($user_face) == 0)
                    {
                        ...
                        if(!empty($facebook_id)){
                            $this->load->model('repository_m');
                            $this->load->model('file_m');
                            $this->load->helpers('MY_form_helper');
                            $this->load->library('uploadHandler', array('initialize'=>FALSE));
...
//                                    echo "unset\r\n";
    ...
    public function ajax ($page_id)
    {
        ...
        if(!empty($search_array['v_search_option_location']) && empty($search_array['v_search_option_81_from']) && empty($search_array['v_search_option_82_to'])){
            $search_array['v_search_option_82_from'] = date('Y-m-d H:i:s');
        } elseif(!empty($search_array)){
				    $search_array['v_search_option_82_from'] = date('Y-m-d H:i:s');
				}
        elseif(empty($search_array['v_search_option_81_from']) && empty($search_array['v_search_option_82_to'])) {
	...
	public function index ()
	{
        ...
        if($search_array === NULL)$search_array  = new stdClass();
            ...
            $search_array->v_search_option_82_from = date('Y-m-d H:i:s');
			  ...
			  elseif( count((array)$search_array) !== 0 && (!empty($search_array->v_search_option_location)
				|| !empty($search_array->v_search_option_smart))
				){
					  $search_array->v_search_option_82_from = date('Y-m-d H:i:s');
				}
        elseif(empty($search_array->v_search_option_81_from) && empty($search_array->v_search_option_82_to)) {
