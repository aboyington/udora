(function($) {
    $.fn.tree_structure = function(options) {
        var defaults = {
            ...
            'base_path' : '',
            'on_change' : null,
            'click_to_add' : 'Click for Add',
            'click_to_edit' : 'Click for Edit',
            'click_to_delete' : 'Click for Delete',
            'first_name' : 'Title',
            'submit' : 'Submit'
        };
        return this.each(function() {
            ...
            var base_path = defaults['base_path'];
            var onChange = defaults['on_change'];
            ...
            var add_action_text = add_option == true ? '<span class="add_action" title="'+defaults['click_to_add']+'"></span>' : '';
            var edit_action_text = edit_option == true ? '<span class="edit_action" title="'+defaults['click_to_edit']+'"></span>' : '';
            var delete_action_text = delete_option == true ? '<span class="delete_action" title="'+defaults['click_to_delete']+'"></span>' : '';
            ...
            if (align_option != 'center')
                $('#' + id_val + ' li').css({'text-align': align_option});
            if (fullwidth_option) {
//                var i = 0;
//                var prev_width;
//                var get_element;
                my_width = 0;
                $('#' + id_val + ' li').each(function() {
                    my_width += $(this).width()+40;
                });
                
//                $('#' + id_val + ' li li').each(function() {
...
//                console.log('fullwidth: '+fullwidth);
                ...
                //$('#' + id_val + '').closest('div').width(fullwidth);
                if(my_width > $('#' + id_val + '').parent().width())
                $('#' + id_val + '').parent().width(my_width);
            }
            
            $('#' + id_val + ' li.thide').each(function() {
            ...
            function draw_line(target) {
                var tree_offset_left = $('#' + id_val + '').offset().left;
            ...
            if (animate_option[0] == true) {
                ...
                var length = $('#' + id_val + ' li').length;
                var i = 0;
                function animate_li() {
                    prepend_data($('#' + id_val + ' li').eq(i));
                    draw_line($('#' + id_val + ' li').eq(i));
                    i++;
                    if (i == length) {
                        i = 0;
            ...
            $('li.lang a').click(function() {
                setTimeout(function(){ call_structure(); }, 500);
            });
            
            $(document).on("click", '#' + id_val + ' b.thide', function() {
                $(this).toggleClass('tshow');
                $(this).closest('li').toggleClass('thide').children('ul').toggle();
                call_structure();
            });
            $(document).on("hover", '#' + id_val + ' li > div', function(event) {
                if (event.type == 'mouseenter' || event.type == 'mouseover') {
                    $('#' + id_val + ' li > div.current').removeClass('current');
                    $('#' + id_val + ' li > div.children').removeClass('children');
                    $('#' + id_val + ' li > div.parent').removeClass('parent');
            ...
            $(document).on("click", '#' + id_val + ' span.highlight', function() {
                $('#' + id_val + ' li.highlight').removeClass('highlight');
                $('#' + id_val + ' li > div.parent').removeClass('parent');
                $('#' + id_val + ' li > div.children').removeClass('children');
            ...
            $(document).on("click", '#' + id_val + ' span.highlight', function() {
                if (fullwidth_option)
                    $('#' + id_val + '').parent('div').parent('div').scrollLeft(0);
                $('#' + id_val + ' li > div').not(".parent, .current, .children").closest('li').addClass('tnone');
                $('#' + id_val + ' li div b.thide.tshow').closest('div').closest('li').children('ul').addClass('tshow');
                $('#' + id_val + ' li div b.thide').addClass('tnone');
                if ($('.back_btn').length == 0) {
                    $('body').prepend('<img src="'+base_path+'images/back.png" class="back_btn" />');
                }
                call_structure();
                $('.back_btn').click(function() {
                    $('#' + id_val + ' ul.tshow').removeClass('tshow');
                    $('#' + id_val + ' li.tnone').removeClass('tnone');
                    $('#' + id_val + ' li div b.thide').removeClass('tnone');
                    $(this).remove();
                    call_structure();
                });
            });
            ...
            if (add_option) {
                $(document).on("click", '#' + id_val + ' span.add_action', function() {
                    ...
                    if (_this.closest('div').children('form.add_data').length == 0) {
                        var addquery = '<form class="add_data" method="post" action=""><img class="close" src="'+base_path+'images/close.png" /><input type="text" class="first_name" name="first_name" placeholder="'+defaults['first_name']+'"><input type="submit" class="submit" name="submit" value="'+defaults['submit']+'"></form>';
                        ...
                        if ((_this.closest('div').children('form').offset().top + _this.closest('div').children('form').outerHeight()) > $(window).height()) {
                            _this.closest('div').children('form').css({'margin-top': 0});
                        }
...
//                        data += _addthis.closest('form').serialize();
                        _addthis.closest("li").before("<img src='"+base_path+"images/load.gif' class='load' />");
...
//                        });
            ...
            if (edit_option) {
                $(document).on("click", '#' + id_val + ' span.edit_action', function() {
                    if ($('form.add_data').length > 0)
                        $('form.add_data').remove();
                    if ($('form.edit_data').length > 0)
                        $('form.edit_data').remove();
                    ...
                    var _this = $(this);
                    var editquery = '<form class="edit_data" method="post" action=""><img class="close" src="'+base_path+'images/close.png" /><input type="text" class="first_name" name="first_name" value="" placeholder="'+defaults['first_name']+'"><input type="submit" class="edit" name="submit" value="'+defaults['submit']+'"></form>';
...
//                        data += _editthis.closest('form').serialize();
                        _editthis.closest("li").before("<img src='"+base_path+"images/load.gif' class='load' />");
...
//                        });
            ...
            if (delete_option) {
                $(document).on("click", '#' + id_val + ' span.delete_action', function() {
                    ...
                    if ($(this).closest('div').attr('id') == 1) {
                        ...
                        if (confirm_message) {
                            ...
                            $(this).closest("li").before("<img src='"+base_path+"images/load.gif' class='load' />");
                            $("img.load").remove();
...
//                                });
                ...
                $('#' + id_val + ' li > div').disableSelection();
                draggable_event();
            }
        });
    };
